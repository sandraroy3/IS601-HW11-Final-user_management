...
#     user_id: UUID,
#     db: AsyncSession = Depends(get_db),
#     current_user: dict = Depends(get_current_user),  # Use the decoded token from get_current_user
# ) -> UserResponse:
#     print("current_user type:", type(current_user), "value:", current_user)
#     print("user_id type:", type(user_id), "value:", user_id)
#     print("current iser id:", current_user["user_id"])
#     # 🔒 Authorization: allow only self or admin/manager
#     current_user_email = current_user["user_id"]  # really the email here

#     db_user = await get_user_by_email(db, current_user_email)
#     if not db_user:
#         raise HTTPException(status_code=404, detail="User not found")

#     if current_user["role"] == "AUTHENTICATED" and db_user.id != user_id:
#         raise HTTPException(
#             status_code=403,
#             detail="Authenticated users can only upload their own profile picture."
#         )
#     # if current_user["role"] == "AUTHENTICATED" and current_user["user_id"] != user_id:
#     #     raise HTTPException(status_code=403, detail="Authenticated users can only upload their own profile picture.")
#     elif current_user["role"] not in {"ADMIN", "MANAGER", "AUTHENTICATED"}:
...
#     # 4. Upload to MinIO
#     bucket_name = get_settings().minio_bucket_name
#     object_name = f"{user_id}/{file.filename}"  # enforce consistent naming
...
#     # 5. Update DB with new profile picture URL
